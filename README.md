...
В сводной таблице кликаем по количеству файлов пользователя - получаем файлы пользователя .
 ...
 - [базовый интерфейс для файлов](#интерфейс-пользователя-c-файлами) из профиля пользователя;
 - [x] '`Quantity files of user`' - ссылка для переходя к файлам пользователя (сводная таблица);
 - [x] '`User status'` - Статус пользователя (администратор или просто пользователь - сводная таблица);
 - [x] '`Change status`' - Выбираем пользователей для изменения статуса (администратор или просто пользователь - сводная таблица);
 - `'Delete'` -удалить одного или более пользователей;
...
![user removing](./img/remove_user.png)

За счёт checkbox выбираем одного, более одного или всех разом пользователей.\
Появляется кнопка '`Delete`' и отправляем на удаление.

#### Note
'`Delete`' Следить за количеством выбранного. \
По умолчани '`Delete`' не видим на странице (css). \
Выбираем одного или всех сразу и '`Delete`' появляется. \
Исключаем всех сразу или последнего из своего выбора и '`Delete`' изчезает. 
